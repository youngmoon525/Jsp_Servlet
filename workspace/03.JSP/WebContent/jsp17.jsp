...
//현재 페이지에서만 사용할 바인딩 객체 생성 : EL 문법(표현식)에 사용하기 위해서
...
<%-- dto 객체를 jsp18.jsp 넘기자 : 동적 페이지 전환(forward()) --%>
<%
//현재 페이지와 요청한 페이지에서 사용할 바인딩(연결) 객체 : request scope
request.setAttribute("dto", dto);
//RequestDispatcher rd = request.getRequestDispatcher("jsp18.jsp");	//페이지 호출
...
<body>
○ 이름(JAVA Variable) : <%=name %><br/>
○ 이름(JAVA dto) : <%=dto.getName() %><br/>
○ 이름(JAVA actionDTO) : <%=actionDTO.getName() %><br/>
<br/>
○ 아이디(Action Tag Variable) : 매개변수로 전달 받은 상태에서는 사용 불가능<br/>
